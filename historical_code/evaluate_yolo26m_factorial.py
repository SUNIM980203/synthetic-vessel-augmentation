#!/usr/bin/env python3
"""Evaluate all accepted YOLO26m cells in the frozen domain order."""
...
EVALUATOR_VERSION = "yolo26m_scope_lr_factorial_ap_evaluator_v1"
...
def load_ledger(jobs: list[dict]) -> dict:
    ...
    ledger = {
        "study": "fresh YOLO26m model-size scope x LR factorial evaluation",
...
def evaluate(job: dict, evaluator_sha: str) -> None:
    ...
    metadata = {
        "schema": "yolo26m_scope_lr_factorial_prediction_archive_v1",
        "evaluator_version": EVALUATOR_VERSION,
...
def main() -> int:
    ...
    for ordinal, job in enumerate(ledger["jobs"], 1):
        ...
        save_ledger(ledger)
        print(f"YOLO26M FACTORIAL EVAL {ordinal}/240 {job['domain']} {job['cell_name']}", flush=True)
        try:
    ...
    save_ledger(ledger)
    print(f"YOLO26M FACTORIAL EVALUATION {ledger['status']}: {ledger['completed_jobs']}/240", flush=True)
