#!/usr/bin/env python3
"""Evaluate every accepted fresh factorial cell and retain lossless predictions."""
...
EVALUATOR_VERSION = "scope_lr_factorial_ap_evaluator_v1"
    ...
    def pred_to_json(self, predn: dict, pbatch: dict) -> None:
        ...
        for bbox, score, category in zip(box.tolist(), predn["conf"].tolist(), predn["cls"].tolist()):
            self.jdict.append({
                "image_id": image_id,
                "file_name": path.name,
                "category_id": self.class_map[int(category)],
...
def load_ledger(jobs: list[dict]) -> dict:
    ...
    ledger = {
        "study": "fresh scope x LR factorial evaluation",
...
def evaluate(job: dict, evaluator_sha: str) -> None:
    ...
    metadata = {
        "schema": "scope_lr_factorial_prediction_archive_v1",
        "evaluator_version": EVALUATOR_VERSION,
        ...
        "prediction_fields": ["image_id", "category_id", "bbox", "score"],
    }
...
def main() -> int:
    ...
    for ordinal, job in enumerate(ledger["jobs"], 1):
        ...
        save_ledger(ledger)
        print(f"FACTORIAL EVAL {ordinal}/240 {job['domain']} {job['cell_name']}", flush=True)
        try:
    ...
    save_ledger(ledger)
    print(f"FACTORIAL EVALUATION {ledger['status']}: {ledger['completed_jobs']}/240", flush=True)
