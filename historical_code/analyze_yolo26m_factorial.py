#!/usr/bin/env python3
"""Frozen YOLO26m factorial contrasts, intervals, and exact sign tests."""
...
OUTPUT_JSON = common.STATISTICS_DIR / "yolo26m_factorial_statistics.json"
PER_SEED_CSV = common.STATISTICS_DIR / "yolo26m_factorial_per_seed_contrasts.csv"
RAW_CSV = common.METRICS_DIR / "yolo26m_factorial_metrics_long.csv"
ABSOLUTE_CSV = common.TABLES_DIR / "yolo26m_factorial_absolute_AP.csv"
...
def main() -> int:
    ...
    payload = {
        "study": "fresh YOLO26m condition x scope x learning-rate factorial",
        ...
        "primary_decision_rule": "outcome-neutral estimation; retain mean, paired values, two-sided Student-t 95% CI, one-sample t-test, and exact two-sided sign test",
        "primary_ci_relation_to_zero": (
            "positive" if primary["two_sided_95_ci"][0] > 0.0
            else "negative" if primary["two_sided_95_ci"][1] < 0.0
            else "includes_zero"
        ),
        "analysis": analysis,
    ...
    print(json.dumps({"primary": primary, "ci_relation_to_zero": payload["primary_ci_relation_to_zero"]}, indent=2), flush=True)
    return 0
